...
namespace Frame3ddn.Test
{
    ...
    public class SolverTest
    {
        ...
        {
            // Historical TestData/testcases-2018 fixtures: NT-prefixed derivatives of the
            // upstream exB/exE/exF/exG/exI examples (geometric stiffness off, dynamic-analysis
            // section trimmed, occasionally fewer load cases) plus B (modified exC). The .out
            // references were generated by the C frame3dd binary on those modified inputs.
            string inputPath = GetTestCases2018Path(fileName + ".csv");
            string referencePath = GetTestCases2018Path(fileName + ".out");
            ...
            // The reference .out may carry more load cases than the (sometimes-trimmed) input
            // produced; clip to what we ran.
            ...
            if (DebugSnapshot.Enabled)
            {
                ...
                int outputStart = outputCalculated.TextOutput.IndexOf(actualOutSection, StringComparison.Ordinal);
                string preamble = outputStart >= 0 ? outputCalculated.TextOutput.Substring(0, outputStart) : "";
                DebugSnapshot.WriteText(classDir, fileName + ".2018.expected.out",
                    preamble + OutWriter.OutputDataToString(reference));
                DebugSnapshot.WriteText(classDir, fileName + ".2018.actual.out",
                    preamble + actualOutSection);
            }

            bool referenceHasPeak = reference.Any(lc => lc.PeakFrameElementInternalForces.Count > 0);
            OutputAsserts.OutputAssertEqual(new Output("", reference), outputCalculated,
                new OutputAssertOptions { IgnorePeakForces = !referenceHasPeak });
        }
        ...
        public void PrescribedDisplacementAppliedAtRestrainedDoF()
        {
            ...
            string inputPath = GetFrame3ddExamplePath("exA.csv");

            using StreamReader sr = new StreamReader(inputPath);
            Input input = CsvParser.Parse(sr);

            Assert.Equal(2, input.LoadCases.Count);
        ...
        public void PrescribedDisplacementValidatesAgainstFreeDoF()
        {
            ...
            string baseInput = File.ReadAllText(GetFrame3ddExamplePath("exA.csv"));
            // Replace the prescribed-disp rows with a non-zero Mz value at node 8 (Mz is free
            // per reactions table). Replaces both LC1 and LC2 occurrences — fine, we just
            // need at least one row to violate the restraint check.
        ...
        private static string GetTestCases2018Path(string fileName)
        {
            string workspaceDir = Directory.GetParent(Directory.GetParent(Directory.GetParent(
                Directory.GetCurrentDirectory().ToString()).ToString()).ToString()).ToString();
            string testDataPath = Directory.GetDirectories(workspaceDir, "TestData")[0];
            return Path.Combine(testDataPath, "testcases-2018", fileName);
        }
