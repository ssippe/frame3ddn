...
{input.LoadCases.Count},,,,,# number of static load cases");
            ...
            {
                LoadCase loadCase = input.LoadCases[i];
...
0,,,,,# number of nodes with prescribed displacements";
        ...
        private static string CreateLoadTrapezoid(LoadCase loadCase)
        {
            ...
            */
            ...
            {
                TrapLoad load = loadCase.TrapLoads[i];
                ...
                sb.AppendLine(
                    $@"{m}, {ToStr(xx1)}, {ToStr(xx2)}, {ToStr(wx1)}, {ToStr(wx2)}, # locations and loads - local x-axis
, {ToStr(xy1)}, {ToStr(xy2)}, {ToStr(wy1)}, {ToStr(wy2)} , # locations and loads - local y-axis
, {ToStr(xz1)}, {ToStr(xz2)}, {ToStr(wz1)}, {ToStr(wz2)}, # locations and loads - local z-axis");
            }
...
M[nU],Ux[nU],Uy[nU],Uz[nU],,,,,,,,,
            ...
            {
                UniformLoad load = loadCase.UniformLoads[i];
                sb.AppendLine(
                    $"{load.ElementIdx + 1}, {ToStr(load.Load.X)}, {ToStr(load.Load.Y)}, {ToStr(load.Load.Z)},");
            }
...
");
            var nodeLines = loadCase.NodeLoads
                .Select(f =>
                    $"{f.NodeIdx}, {ToStr(f.Load.X)}, {ToStr(f.Load.Y)}, {ToStr(f.Load.Z)}, {ToStr(f.Moment.X)}, {ToStr(f.Moment.Y)}, {ToStr(f.Moment.Z)}")
...
# End of input data file";
        ...
        private static string ToStr(int i) => i.ToString();

        private static string ToStr(double d)
        {
            return d.ToString("0.0###############e0");
        }

        private static string CreateHeader(string fileStamp)
        {

            return $@"""Generated by {nameof(InputCsvWriter)} units(N, mm, ton) {fileStamp}""
...
");
            ...
            {
                FrameElement member = input.FrameElements[i];
        ...
        private static string CreateFrameElementLine(int idx, FrameElement frameElement)
        {
            ...
            return $"{ei}, {n1}, {n2}, {ToStr(ax)}, {ToStr(asy)}, {ToStr(asz)}, {ToStr(jxx)}, {ToStr(iyy)}, {ToStr(izz)}, {ToStr(e)}, {ToStr(g)}, {ToStr(roll)}, {ToStr(density)} ";
        }
...
#.,mm,mm,mm,deg");
        ...
        {
            IReadOnlyList<ReactionInput> reactions = input.ReactionInputs;
            var sb = new StringBuilder();
            sb.AppendLine(
                $@"
